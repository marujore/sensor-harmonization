...
# Adapted by Rennan Marujo - rennanmarujo@gmail.com 
...
def CalcGroundVectors( AngleObs, gsd, subsamp, start_row, end_row, start_col, end_col, out_rows, out_cols ):
	GVecs = np.zeros( (int(out_rows), int(out_cols), 3) )
	ul_x = AngleObs['ul_x']
	ul_y = AngleObs['ul_y']
	zone = AngleObs['zone']
	if AngleObs['hemis'] == 'S':
...
def main():
	...
	for tparms in TimeParms:
		...
		# Out_File = Tile_Base[0][:-4] + '_Sat_' + bandName + '.tif'
		...
		print('Created image file {} and header file {}.'.format(Out_File, Hdr_File) )
